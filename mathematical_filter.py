...
class MathematicalFilter:
    ...
    def _find_incident_trigger(self, df: pd.DataFrame, col: str, window_size: int) -> tuple[int | None, float]:
        ...
        rolling_mean = series.rolling(window=window_size*3, min_periods=window_size).mean()
        rolling_std = series.rolling(window=window_size*3, min_periods=window_size).std()
